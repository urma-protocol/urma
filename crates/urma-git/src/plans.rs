...
impl GitPlan {
    ...
    pub fn freeze(
        ...
        publication.save_new(&directory.join("publication.json"))?;
    ...
    pub fn load(directory: &Path) -> Result<(Self, String), Error> {
        ...
        if plan.schema != 1 {
            return Err(Error::Invalid("Git plan schema".into()));
        }
        let names = plan
            .artifacts
            .iter()
        ...
        plan.publication(directory)?;
        Ok((plan, identifier))
    }

    pub fn publication(&self, directory: &Path) -> Result<PublicationPlan, Error> {
        let publication = PublicationPlan::load(&directory.join("publication.json"))?;
        ...
        binding::verify(&publication, directory, &self.limits)?;
        Ok(publication)
    }
}
